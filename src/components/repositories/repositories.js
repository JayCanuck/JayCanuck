        ...
        {status === 'ready' && this.state.repos && (
          <React.Fragment>
              ...
              {this.state.repos.map(repo => (
                <React.Fragment key={repo.name}>
                  <div className='repositories__repo'>
                    <a className='repositories__repo-link' href={repo.html_url} target='_blank' rel='noreferrer'>
                      <strong>{repo.name}</strong>
                    </a>
                    <div>{repo.description}</div>
                    <div className='repositories__repo-date'>Updated: {new Date(repo.updated_at).toUTCString()}</div>
            ...
            <div className='repositories_user-link'>
              <a href={`https://github.com/${siteConfig.githubUsername}`} target='_blank' rel='noreferrer'>
